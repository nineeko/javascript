...
// sort의 기본 정렬은 문자 정렬이므로 필요하다면 정렬 기준 콜백 함수를 전달해야한다.
...
// Array.prototype.filter : 배열 요소 전체를 대상으로 콜백함수 호출 후 반환 값이ㅣ  true인
// 요소로만 구성된 새로운 배열 반환
...
// Array.prototype.every : 배열 내 모든 요소가 3보다 큰지 확인
// 그 결과를 boolean으로 반환
// 배열 내 모든 요소가 보다 큰지 확인
...
// 그 결과가 참인 첫 번째 요소를 반환한다. 참인 요소가 존재하지 않는다면 undefinded를 반환한다.
// Array.prototype.findIndex : 배열을 순회하며 각 요소에 대하여 인자로 주어진 콜백함수를 실행하여
// 그 결과가 참인 첫 번째 요소의 인덱스를 반환한다. 참인 요소가 존재하지 않는다면 -1를 반환한다.
const students = [
    {name: '유관순', score: 90},
    {name: '홍길동', score: 80},
    {name: '장보고', score: 70}
